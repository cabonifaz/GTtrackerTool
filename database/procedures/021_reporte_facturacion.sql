...
-- Igual que sp_reporte_facturacion_mensual, pero usando SIEMPRE el
-- perfil/tarifa vigente HOY (fecha_hasta IS NULL) para todo el mes, en
-- vez de la vigente historicamente a la fecha de corte -- para el boton
-- "Regenerar con tarifas actuales" cuando se corrigio una tarifa/perfil
-- despues de que el periodo ya paso. Solo tiene sentido en un mes que
-- todavia no esta cerrado (una vez cerrado, el export usa el snapshot
-- congelado y este SP no se llama).
DROP PROCEDURE IF EXISTS sp_reporte_facturacion_mensual_tarifa_actual $$
CREATE PROCEDURE sp_reporte_facturacion_mensual_tarifa_actual(
  IN p_id_proyecto      INT UNSIGNED,
  IN p_anio             INT,
  IN p_mes              INT,
  IN p_id_empresa_actor INT UNSIGNED
...
BEGIN
  ...
  perfil_vigente AS (
    -- Unica diferencia con sp_reporte_facturacion_mensual: perfil/tarifa
    -- vigente HOY, no a la fecha de corte del mes reportado.
    SELECT a.id_usuario, upp.id_perfil
    FROM asignados a
    JOIN usuarios_proyectos_perfiles upp
      ON upp.id_usuario_proyecto = a.id_usuario_proyecto
     AND upp.fecha_hasta IS NULL
  ),
  tarifa_vigente AS (
    SELECT pv.id_usuario, pv.id_perfil, pt.tarifa, pt.id_moneda
    FROM perfil_vigente pv
