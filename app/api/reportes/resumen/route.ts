...
import { reporteResumenAvance } from "@/lib/services/reporteService";
...
export async function GET(req: NextRequest) {
  ...
  try {
    const filas = await reporteResumenAvance(Number(idProyecto), Number(anio), Number(mes), session.user.idEmpresa!);
